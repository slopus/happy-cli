...
export class OpenCodeReasoningProcessor {
    ...
    processChunk(chunk: string): void {
        ...
        if (!this.inTitleCapture && !this.hasTitle && !this.contentBuffer) {
            ...
            if (titleEndIndex !== -1) {
                ...
                logger.debug(`[OpenCodeReasoningProcessor] Title captured: "${title}"`);
    ...
    private sendToolCallStart(title: string): void {
        ...
        logger.debug(`[OpenCodeReasoningProcessor] Sending tool call start for: "${title}"`);
    ...
    complete(): boolean {
        ...
        if (!fullText.trim() && !this.toolCallStarted) {
            logger.debug('[OpenCodeReasoningProcessor] Complete called but no content accumulated, skipping');
            return false;
        }
        ...
        logger.debug(`[OpenCodeReasoningProcessor] Complete reasoning - Title: "${title}", Has content: ${content.length > 0}`);
        ...
        if (this.toolCallStarted && this.currentCallId) {
            ...
            logger.debug('[OpenCodeReasoningProcessor] Sending tool call result');
            ...
            logger.debug('[OpenCodeReasoningProcessor] Sending reasoning message');
     ...
     */
    abort(): void {
        logger.debug('[OpenCodeReasoningProcessor] Abort called');
        this.finishCurrentToolCall('canceled');
        this.resetState();
    }
    ...
    private finishCurrentToolCall(status: 'completed' | 'canceled'): void {
        if (this.toolCallStarted && this.currentCallId) {
            ...
            logger.debug(`[OpenCodeReasoningProcessor] Sending tool call result with status: ${status}`);
            this.onMessage?.(toolResult);
        }
    }
